...
export const assemble = (ops: AssemblerOperation[]) => {
  ...
  const resolveSymbol = (symbol: SymbolOr<ImmediateValue>): ResolutionResult => {
    if (symbol.type === 'symbolReference') {
      ...
      const a = resolveSymbol(symbol.symbolA);
      const b = resolveSymbol(symbol.symbolB);
  ...
  // TODO: In order to support compound operations, I'll need to extract the following logic
  // to a function that can recurse
  for (let op of ops) {
    switch (op.type) {
      case "moveTo": {
        offset = op.address;
      ...
      case "opDescription": {
        if (op.isPrefix) {
          ROMBuffer[offset++] = 0xCB;
        }
        ROMBuffer[offset++] = op.opcode;
        ...
        // One Byte immediates
        if ('u8' in op) {
          const result = resolveSymbol(op.u8);
          if (result.resolved) {
            ROMBuffer[offset] = result.value & 0xff;
          } else {
            revisit.push({ offset, size: 1, symbol: op.u8 as SymbolReference });
          }
          offset += 1;
        } else if ('i8' in op) {
          const result = resolveSymbol(op.i8);
          if (result.resolved) {
            ROMBuffer[offset] = result.value & 0xff;
          } else {
          ...
          const result = resolveSymbol(op.ffPageOffset);
          if (result.resolved) {
            ROMBuffer[offset] = result.value & 0xff;
          } else {
          ...
          const result = resolveSymbol(op.spOffset);
          if (result.resolved) {
            ROMBuffer[offset] = result.value & 0xff;
          } else {
        ...
        // Two Byte Immediates
          ...
          const result = resolveSymbol(op.u16);
          if (result.resolved) {
            ROMBuffer[offset] = result.value & 0xff;
            ROMBuffer[offset + 1] = (result.value >> 8) & 0xff;
          } else {
            revisit.push({ offset, size: 2, symbol: op.u16 as SymbolReference });
          }
          offset += 2;
        } else if ('u16ptr' in op) {
          const result = resolveSymbol(op.u16ptr);
          if (result.resolved) {
            ROMBuffer[offset] = result.value & 0xff;
            ROMBuffer[offset + 1] = (result.value >> 8) & 0xff;
          } else {
  ...
  for (const item of revisit) {
    const result = resolveSymbol(item.symbol);
    if (!result.resolved) {
